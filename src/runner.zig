...
fn runMacos(app: native_sdk.App, options: RunOptions, init: std.process.Init) !void {
    ...
    const mac_platform = try native_sdk.platform.macos.MacPlatform.createWithOptions(startupSurfaceSize(app_info), webEngine(), app_info);
...
fn runLinux(app: native_sdk.App, options: RunOptions, init: std.process.Init) !void {
    ...
    const linux_platform = try native_sdk.platform.linux.LinuxPlatform.createWithOptions(startupSurfaceSize(app_info), webEngine(), app_info);
...
fn runWindows(app: native_sdk.App, options: RunOptions, init: std.process.Init) !void {
    ...
    const windows_platform = try native_sdk.platform.windows.WindowsPlatform.createWithOptions(startupSurfaceSize(app_info), webEngine(), app_info);
...
fn startupSurfaceSize(app_info: native_sdk.AppInfo) native_sdk.geometry.SizeF {
    const frame = app_info.resolvedMainWindow().default_frame;
    const width = if (std.math.isFinite(frame.width) and frame.width > 0) frame.width else 1440;
    const height = if (std.math.isFinite(frame.height) and frame.height > 0) frame.height else 960;
    return native_sdk.geometry.SizeF.init(width, height);
}
    ...
    if (app_info.windows.len > 0) {
        ...
        for (restored_windows, 0..) |*window, index| {
            ...
            if (store.loadWindow(window.label, &buffers.read) catch null) |saved| {
                if (savedFrameUsable(window.*, saved.frame)) {
        ...
        if (store.loadWindow(app_info.main_window.label, &buffers.read) catch null) |saved| {
            if (savedFrameUsable(app_info.main_window, saved.frame)) {
                app_info.main_window.default_frame = saved.frame;
                app_info.main_window.initial_placement = .restored;
            }
        }
...
fn savedFrameUsable(window: native_sdk.WindowOptions, frame: native_sdk.geometry.RectF) bool {
    // A stale state file from the SDK default (720x480) must not override the
    // app manifest's larger startup frame. Keep valid user-resized frames.
    return frame.width >= window.min_width and frame.height >= window.min_height;
}

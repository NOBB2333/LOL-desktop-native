...
const props = defineProps<{ match: MatchSummary; highlightedPuuids?: string[] }>();
...
const sourceLabel = (source: string) => source === "lcu" ? "客户端实时" : source === "sgp" ? "服务器战绩" : source === "fixture" ? "预览数据" : source.includes("sqlite") ? "本地缓存" : "来源暂不可用";
...
<template>
  <article class="history-detail" :data-result="isUnfinished(match) ? 'unfinished' : isWin(match) ? 'win' : 'loss'">
    <header class="history-detail__header">
      <div>
        <span class="eyebrow">对局详情 / {{ shortDate(match.playedAt) }}</span>
    ...
    <section class="history-detail__hero">
      ...
      <div class="history-detail__headline"><span>{{ match.mvp ?? "本局表现" }}</span><strong>{{ match.performance === "carry" ? "出色" : match.performance === "carried" ? "躺赢" : match.performance === "struggling" ? "低迷" : "正常" }}</strong><small>{{ Math.round(match.killParticipation * 100) }}% 参团</small></div>
    ...
    <section class="history-detail__teams">
      <div class="history-detail__section-heading"><div><span class="eyebrow">玩家与禁选</span><h3>十人完整数据</h3></div><span>队伍击杀 {{ match.teamKills }} · 下载可留存全部字段</span></div>

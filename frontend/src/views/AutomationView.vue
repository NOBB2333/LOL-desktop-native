...
<template>
  <div class="page-shell auto-page">
    ...
    <section class="panel shortcut-panel">
      ...
      <div class="shortcut-settings"><label><span>发送防误触<small>单条输入期间短暂屏蔽鼠标和键盘</small></span><NSwitch v-model:value="app.config.automation.protectChatInput" size="small" /></label><label><span>多条消息间隔<small>默认 250ms；发送异常时可适当调高</small></span><NInputNumber v-model:value="app.config.automation.shortcutSendIntervalMs" :min="250" :max="5000" :step="50" size="small"><template #suffix>ms</template></NInputNumber></label><label><span>逐场战绩数量<small>仅影响 {recent_games}</small></span><NInputNumber v-model:value="app.config.automation.shortcutRecentGameCount" :min="1" :max="10" size="small"><template #suffix>场</template></NInputNumber></label></div>

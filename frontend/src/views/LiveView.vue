...
/// 设置页那条「仅显示排位数据」，挪一份到这里方便临场切换。切换后本页已有的
/// `watch(rankedOnly)` 会重建对局快照，战绩查询的 key 里也带了这个开关，会自动重取。
const rankedOnlyHint = computed(() => app.config.providers.rankedOnly ? "只看单双排与灵活组排" : "显示全部模式");
...
/* 数据范围：把设置页的「仅显示排位数据」搬一份过来，省得为了切一下跑设置页。 */
.game-quick-filter {
  display: grid;
  gap: 6px;
  margin-top: 10px;
...
.game-quick-filter__title {
  color: var(--text-muted);
  font-size: 9px;
  letter-spacing: 0.08em;
}
.game-quick-filter__row {
  display: grid;
  grid-template-columns: minmax(0, 1fr) auto;
  align-items: center;
  gap: 6px;
}
.game-quick-filter__row strong {
  display: block;
  font-size: 10px;
  font-weight: 600;
}
.game-quick-filter__row small {
  display: block;
  overflow: hidden;
  color: var(--text-muted);
  font-size: 9px;

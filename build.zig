...
const app_exe_name = "lol-desktop-native";
...
const NativeConfig = struct {
    ...
    nativeSdkPath: []const u8 = default_native_sdk_path,
    dataDirName: []const u8 = "lol-desktop-native",
    lcu: struct {
        lockfilePaths: []const []const u8 = &.{},
        requestTimeoutMs: u32 = 6000,
        liveProbeTimeoutMs: u32 = 900,
        verifyTls: bool = false,
        liveClientDataUrl: []const u8 = "https://127.0.0.1:2999/liveclientdata/allgamedata",
    } = .{},
    windows: struct {
...
fn nativeConfig(b: *std.Build) NativeConfig {
    return std.json.parseFromSliceLeaky(NativeConfig, b.allocator, @embedFile("config/native.json"), .{ .ignore_unknown_fields = true }) catch
        @panic("config/native.json is invalid JSON");
}

fn parsePackageTarget(value: []const u8) PackageTarget {
...
pub fn build(b: *std.Build) void {
    ...
    const package_target = b.option(PackageTarget, "package-target", "Package target: macos, windows, linux") orelse parsePackageTarget(config.package.target);
    const package_archive = b.option(bool, "package-archive", "Create a distributable archive") orelse config.package.archive;
    const native_sdk_path = b.option([]const u8, "native-sdk-path", "Path to the Native SDK framework checkout") orelse config.nativeSdkPath;
    ...
    options.addOption([]const u8, "data_dir_name", config.dataDirName);
    options.addOption([]const u8, "dev_server_host", config.devServer.host);
    options.addOption(u16, "dev_server_port", config.devServer.port);
    options.addOption(u32, "lcu_request_timeout_ms", config.lcu.requestTimeoutMs);
    options.addOption(u32, "lcu_live_probe_timeout_ms", config.lcu.liveProbeTimeoutMs);
    options.addOption(bool, "lcu_verify_tls", config.lcu.verifyTls);
    options.addOption([]const []const u8, "lcu_lockfile_paths", config.lcu.lockfilePaths);
    options.addOption([]const u8, "lcu_live_client_data_url", config.lcu.liveClientDataUrl);
    options.addOption([]const u8, "update_check_url", config.updates.checkUrl);
    const options_mod = options.createModule();

    const generated_app_manifest_mod = appManifestModule(b);
    ...
    runner_mod.addImport("app_manifest_zon", generated_app_manifest_mod);
    ...
    app_mod.addImport("app_manifest_zon", generated_app_manifest_mod);
    const lcu_mod = localModule(b, target, optimize, "src/lcu.zig");
    app_mod.addImport("lcu", lcu_mod);
    const backend_mod = localModule(b, target, optimize, "src/backend.zig");
    backend_mod.addImport("native_sdk", native_sdk_mod);
    backend_mod.addImport("lcu", lcu_mod);
    backend_mod.addImport("build_options", options_mod);
    const storage_mod = localModule(b, target, optimize, "src/storage.zig");
    storage_mod.addIncludePath(nativeSdkPath(b, native_sdk_path, "third_party/sqlite"));
    backend_mod.addImport("storage", storage_mod);
    app_mod.addImport("backend", backend_mod);
    const bridge_mod = localModule(b, target, optimize, "src/bridge.zig");
    bridge_mod.addImport("native_sdk", native_sdk_mod);
    bridge_mod.addImport("backend", backend_mod);
    bridge_mod.addImport("build_options", options_mod);
    app_mod.addImport("bridge", bridge_mod);
    if (app_config.sqlite_capability) addSqliteEngine(b, app_mod, native_sdk_path);
    if (target.result.os.tag == .windows) app_mod.addWin32ResourceFile(.{ .file = b.path("assets/windows.rc") });
    addMacosInfoPlist(b, app_mod, target, app_config);
    const exe = b.addExecutable(.{
        .name = app_exe_name,
        .root_module = app_mod,
        // League commonly runs elevated. Match the Rust release host so
        // Windows does not silently reject SendInput for in-game chat.
        .win32_manifest = if (target.result.os.tag == .windows and optimize != .Debug and config.windows.requireAdministrator) b.path("assets/app.manifest") else null,
    ...
    // Package the web UI (and the system WebView2 loader on Windows) into
    // the executable. The Native host still serves files from a directory,
    // so the binary extracts this bundle to a per-user runtime cache at
    // startup; distribution can therefore be a single .exe.
    const embed_assets = b.addSystemCommand(&.{"node"});
    embed_assets.addFileArg(b.path("scripts/embed-assets.mjs"));
    embed_assets.addDirectoryArg(b.path("frontend/dist"));
    // Directory arguments establish the path but do not recursively hash
    // their contents in Zig 0.16. Track the source/public trees explicitly so
    // a warm build regenerates the bundle when a fixture or frontend module
    // changes, even though `frontend/dist` itself is generated output.
    addFrontendAssetInputs(b, embed_assets, "frontend/src");
    addFrontendAssetInputs(b, embed_assets, "frontend/public");
    for ([_][]const u8{ "frontend/index.html", "frontend/vite.config.js", "frontend/package.json", "frontend/package-lock.json" }) |path| {
        if (b.build_root.handle.access(b.graph.io, path, .{})) |_| embed_assets.addFileInput(b.path(path)) else |_| {}
    }
    if (target.result.os.tag == .windows and web_layer)
        embed_assets.addFileArg(nativeSdkPath(b, native_sdk_path, webView2LoaderSubPath(target)))
    else
        embed_assets.addArg("-");
    // The frontend bundle is not the only runtime file needed by the host:
    // tray/status-item APIs consume a filesystem icon. Include the source
    // icon in the same LDF bundle so the copied single exe has no cwd assets.
    embed_assets.addFileArg(b.path("assets/icon.png"));
    if (target.result.os.tag == .windows) embed_assets.addFileArg(b.path("assets/icon.ico"));
    const embedded_bundle = embed_assets.addOutputFileArg("frontend.bundle");
    const embedded_source = embed_assets.addOutputFileArg("embedded_assets.zig");
    _ = embedded_bundle;
    embed_assets.step.dependOn(&frontend_build.step);
    const embedded_assets_mod = b.createModule(.{ .root_source_file = embedded_source });
    app_mod.addImport("embedded_assets", embedded_assets_mod);
    ...
    const package_exe = if (package_optimize == optimize) exe else pkg: {
        ...
        const package_app_manifest_mod = appManifestModule(b);
        ...
        package_runner_mod.addImport("app_manifest_zon", package_app_manifest_mod);
        ...
        package_app_mod.addImport("app_manifest_zon", package_app_manifest_mod);
        const package_lcu_mod = localModule(b, target, package_optimize, "src/lcu.zig");
        package_app_mod.addImport("lcu", package_lcu_mod);
        const package_backend_mod = localModule(b, target, package_optimize, "src/backend.zig");
        package_backend_mod.addImport("native_sdk", package_sdk_mod);
        package_backend_mod.addImport("lcu", package_lcu_mod);
        package_backend_mod.addImport("build_options", options_mod);
        const package_storage_mod = localModule(b, target, package_optimize, "src/storage.zig");
        package_storage_mod.addIncludePath(nativeSdkPath(b, native_sdk_path, "third_party/sqlite"));
        package_backend_mod.addImport("storage", package_storage_mod);
        package_app_mod.addImport("backend", package_backend_mod);
        const package_bridge_mod = localModule(b, target, package_optimize, "src/bridge.zig");
        package_bridge_mod.addImport("native_sdk", package_sdk_mod);
        package_bridge_mod.addImport("backend", package_backend_mod);
        package_bridge_mod.addImport("build_options", options_mod);
        package_app_mod.addImport("bridge", package_bridge_mod);
        package_app_mod.addImport("embedded_assets", embedded_assets_mod);
        if (app_config.sqlite_capability) addSqliteEngine(b, package_app_mod, native_sdk_path);
        if (target.result.os.tag == .windows) package_app_mod.addWin32ResourceFile(.{ .file = b.path("assets/windows.rc") });
        addMacosInfoPlist(b, package_app_mod, target, app_config);
        const built = b.addExecutable(.{
            .name = app_exe_name,
            .root_module = package_app_mod,
            .win32_manifest = if (target.result.os.tag == .windows and package_optimize != .Debug and config.windows.requireAdministrator) b.path("assets/app.manifest") else null,
    ...
    // output argument. Always run this inexpensive staging step so removing
    // the previous package directory cannot leave a false cache hit, while
    // the expensive executable compilation remains cached normally.
    ...
    // Backend is a test root here, so it needs its own SQLite translation
    // unit; the application links SQLite from `app_mod`, which is not part of
    // this standalone test artifact.
    const backend_test_mod = localModule(b, target, optimize, "src/backend.zig");
    backend_test_mod.addImport("native_sdk", native_sdk_mod);
    backend_test_mod.addImport("lcu", lcu_mod);
    backend_test_mod.addImport("build_options", options_mod);
    const storage_test_mod = localModule(b, target, optimize, "src/storage.zig");
    storage_test_mod.addIncludePath(nativeSdkPath(b, native_sdk_path, "third_party/sqlite"));
    ...
    test_step.dependOn(&b.addRunArtifact(lcu_tests).step);
...
fn addFrontendAssetInputs(b: *std.Build, run: *std.Build.Step.Run, src_path: []const u8) void {
    var dir = b.build_root.handle.openDir(b.graph.io, src_path, .{ .iterate = true }) catch return;
    defer dir.close(b.graph.io);
    var walker = dir.walk(b.allocator) catch return;
    defer walker.deinit();
